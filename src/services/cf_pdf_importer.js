...
module.exports = function () {
  ...
  function cropProblemUpperHeader(folder, data, upper, bottom, i, callback) {
    ...
    return exec(
      `pdfcrop --margins '0 -${upper} 0 -${bottom}' ${folder}/${page}.pdf ${folder}/${page}.pdf`,
      callback
    );
  }
    ...
    async.series(
      [
        (next) => {
          ...
          async.timesSeries(
            problemPageCount,
            cropProblemUpperHeader.bind(null, folder, data, 0, bottomOffset),
            next
          );
        },
        (next) => {
          async.timesSeries(
            problemPageCount,
            cropProblemUpperHeader.bind(null, folder, data, upperOffset, 0),
            next
          );
        },
        (next) => {
          ...
          exec(
            `pdfjam ${pdfs} --nup 1x${problemPageCount} --outfile ${folder}/p${idx}.pdf`,
            next
          );
        },
        (next) => {
        ...
        (next) => {
          ...
          exec(
            `pdfcrop --margins '0 -${metadataOffset} 0 0' ${pdf} ${pdf}`,
            next
          );
        },
      ],
  ...
  function hasMetadata(texts) {
    ...
    try {
      metadata = _.chain(texts)
        .map((o) => {
          if (/^\s*Problema?\s*([\w\d]{0,3})[.:]\s*(.+)?\s*$/.exec(o))
            return "name";
  ...
  function importProblem(language, folder, data, problem, i, callback) {
    S3.upload(
      ...
      (err, details) => {
        ...
        console.log(
          `>>> Imported in batch ${problem.id} from ${problem.oj} (${problem._id}). ${problem.url}`
        );
    ...
    async.waterfall(
      [
        ...
        (stdout, stderr, next) => {
          ...
          async.timesSeries(
            numberOfPages,
            checkProblemName.bind(null, allPdfFile),
            next
          );
        },
        (_data, next) => {
        ...
        (stdout, stderr, next) => {
          hasFooter = false;
          try {
            let text = getPDFTextSync(
              `${folder}/${problemsIdx[0].startPage}.pdf`
            );
            text = _.split(text, "\n");
          ...
          upperOffset = getUpperOffset(
            `${folder}/${problemsIdx[0].startPage}.pdf`
          );
          let langs = LangDetector.detect(
            getPDFTextSync(`${folder}/${problemsIdx[0].startPage}.pdf`)
          );
          language = (langs && langs.length > 0 && langs[0]) || "english";
          async.eachOfSeries(
            problemsIdx,
            generateProblemPDF.bind(null, folder),
            next
          );
        },
      ],
    ...
    async.waterfall(
      [
        ...
        (_folder, language, data, next) => {
          ...
          async.eachOf(
            problems,
            importProblem.bind(null, language, folder, data),
            next
          );
        },
        (next) => {
