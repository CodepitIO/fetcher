...
const PROBLEM_METADATA_API =
  "/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=%s";
...
exports.import = (problem, callback) => {
  let metadataUrl = problem.originalUrl;
  ...
  async.parallel(
    ...
    (err, results) => {
      ...
      try {
        ...
        let tl =
          parseFloat(
            results.meta[1].match(/Time limit: ([\d\.]+) seconds/)[1]
          ) * 1000 || 3000;
        data.timelimit = tl / 1000.0;
        data.memorylimit = "128 MB";
        let html = iconv.decode(results.body[1], "ISO-8859-1");
        data.isPdf = !!html.match(
          /<meta\s+http-equiv=\"Refresh\"\s+content=\".*.pdf\">/i
        );
  ...
  client.get(href, (err, res, html) => {
    ...
    $("tr.sectiontableheader")
      ...
      .each((i, elem) => {
        ...
        let res = PROBLEM_PATTERN.exec(elem.text());
        if (res && res[1] && res[2]) {
          let href = Config.url + "/" + $(elem).find("a").attr("href");
          problems.push({
            id: res[1],
            name: res[2],
            oj: TYPE,
